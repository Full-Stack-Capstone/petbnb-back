  ...
  before do
    ...
    @pet = Pet.create(name: Faker::Creature::Cat.name,
      ...
      @pet_room = PetRoom.create(name: "My Room", 
        ...
        price: 100,
        user_id: @user.id)
      @reservation = Reservation.create( start_date: Faker::Date.between(from: 2.days.ago, to: Date.today),
      end_date: Faker::Date.between(from: Date.today, to: 2.days.from_now),
      pet_id: @pet,
      pet_room_id: @pet_room
    )
  end 

  it 'start_date should be present' do
    expect (@reservation.start_date).to_not be_valid
